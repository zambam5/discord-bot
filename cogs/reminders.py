    ...
    async def remindme(self, ctx):
        ...
        if channel.id in self.channels:
            ...
            if unit not in self.time_units.keys():
                await channel.send(
                    "Unit must be one of days(d), hours(h), minutes(m), or seconds(s); and not a mix of the four"
                )
                return
            ...
            if seconds >= self.limit:
                await channel.send(
                    "That is too far in the future I may be dead <:FeelsStrongMan:795264167107100692>"
                )
                return
            else:
                ...
                await channel.send(
                    name + " I will remind you in {} seconds".format(seconds)
                )
